...
export function AppointmentCard({
  ...
  // Determine if appointment is active (within 15 minutes of start time)
  const isAppointmentActive = () => {
    const now = new Date();
    const appointmentTime = new Date(appointment.startTime);
    const appointmentEndTime = new Date(appointment.endTime);

    // Can join 15 minutes before start until end time
    return (
      (appointmentTime.getTime() - now.getTime() <= 15 * 60 * 1000 &&
        now < appointmentTime) ||
      (now >= appointmentTime && now <= appointmentEndTime)
    );
  };

  const detailAppointmentActive = () => {
    const now = new Date();
    const appointmentTime = new Date(appointment.startTime);
    const appointmentEndTime = new Date(appointment.endTime);
 if (!(now <= appointmentEndTime)) {
    ...
    if (!(appointmentTime.getTime() - now.getTime() <= 15 * 60 * 1000 &&
        now < appointmentTime)) {
          return "Chat akan tersedia 15 menit sebelum jadwal"
        }
